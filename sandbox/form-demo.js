...
function validateForm(event) {
  ...
  if(theForm.name.value.toLowerCase != 'bob'){
    errors.push("Invalid Name")
    isValid = false;
  }
  if(theForm.creditCardNumber.value.length != 0 && theForm.creditCardNumber.value != "1234123412341234"){
...
function togglePaymentDetails(e) {
  ...
  if(formData.get("paymentMethod") == 0){
    creditCardContainer.classList.remove("hide");
    creditCardInput.setAttribute("required", true);
  }else if(formData.get("paymentMethod") == 1){
