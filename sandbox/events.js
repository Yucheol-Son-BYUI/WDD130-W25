...
function renderTasks(tasks) {
  ...
  const list_UL = document.querySelector("#todoList");
...
function taskTemplate(task){
  return `<li ${task.completed ? 'class="strike"' : ""}><p>${task.detail}</p><div><span data-function="delete">❎</span><span data-function="complete">✅</span></div></li>`;
}
...
function manageTasks(event) {
  ...
  if(!li) return; //if there is no li, return
